...
import SearchExercisesComponent from './searchExercisesComponent';
import FilterMuscleModal from './filterMuscleModal'; // Importing the modal
...
import {setBundleNameState,setSelectedExercisesState} from "../../../common/redux/slice/exercisesSlice";

const CreateWorkoutBundles = () => {
  ...
  const handleAddExercise = (exercise) => {
    ...
    if (!selectedExercises.some((selected) => selected.id === exercise.id)) {
      ...
      dispatch(setSelectedExercisesState({selectedExercisesState:updatedSelectedExercises}));
  ...
  const handleRemoveExercise = (exerciseId) => {
    ...
    dispatch(setSelectedExercisesState({selectedExercisesState:updatedExercises}));
  };
  ...
  const handleBundleName = (value) => {
    setBundleName(value);
    dispatch(setBundleNameState({bundleNameState:value}));
  }

  const handleAddBundle = () => {
    
    // API call to save exercise
    // send the bundleName and Selected exercises
    ...
    dispatch(setBundleNameState({bundleNameState:""}));
    dispatch(setSelectedExercisesState({ selectedExercisesState: [] }));
  };
  ...
  useEffect(() => {
    ...
    setSelectedExercises(selectedExercisesState);
    setBundleName(bundleNameState);
  }, []);

  // Get unique muscles from selected exercises
  const uniqueMuscles = [
    ...new Set(
  ...
  return (
    ...
    <div className="mt-4">
      <input
        type="text"
        value={bundleName}
        onChange={(e) => handleBundleName(e.target.value)}
        className="text-black w-full p-3 rounded-md border-2 border-red-500"
      ...
      <button
        ...
        className="bg-green-500 text-black border-2 border-black px-4 py-2 rounded-md mt-2"
      >
        Add to My Workout Bundles
      </button>
    </div>

    {isAreYouSureModalOpen && (
        <AreYouSureModal onConfirm={handleAddBundle} onCancel={handleOnClose} />
      )}
...
export default CreateWorkoutBundles;
